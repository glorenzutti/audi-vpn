...
REMMINA_CNF=$HOME/.remmina/audi.remmina
...
ask_user(){
user=$(zenity --entry --title="Conexión VPN AudiRED - Usuario" --width=250 --ok-label="Aceptar" --cancel-label="Cancelar" --text="¿Cuál es tu nombre de usuario?")
}

ask_pass(){
pass=$(zenity --password --title="Conexión VPN AudiRED - Contraseña" --width=250 --ok-label="Aceptar" --cancel-label="Cancelar" --text="Introduce la contraseña:")
}
...
editsysctl(){
	sudo grep -qxF 'net.ipv6.conf.all.disable_ipv6=1' $SYSCTL || echo 'net.ipv6.conf.all.disable_ipv6=1' >> $SYSCTL
	sudo grep -qxF 'net.ipv6.conf.default.disable_ipv6=1' $SYSCTL || echo 'net.ipv6.conf.default.disable_ipv6=1' >> $SYSCTL
	sudo grep -qxF 'net.ipv6.conf.lo.disable_ipv6=1' $SYSCTL || echo 'net.ipv6.conf.lo.disable_ipv6=1' >> $SYSCTL
	sudo sysctl -p
}
...
(sudo echo "auth-user-pass $OVPN_CREDENTIALS" && sudo cat $file) > $OVPN_CNF
